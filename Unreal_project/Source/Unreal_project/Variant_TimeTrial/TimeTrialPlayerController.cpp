	...
	if (IsLocalPlayerController())
	{
		if (ShouldUseTouchControls())
		{
			...
			if (MobileControlsWidget)
			{
				...
				UE_LOG(LogUnreal_project, Error, TEXT("Could not spawn mobile controls widget."));
		...
		if (UIWidget)
		{
			...
			UE_LOG(LogUnreal_project, Error, TEXT("Could not spawn Time Trial UI widget."));

		}
		

		// spawn the UI widget and add it to the viewport
		VehicleUI = CreateWidget<UUnreal_projectUI>(this, VehicleUIClass);
		...
		if (VehicleUI)
		{
			...
			UE_LOG(LogUnreal_project, Error, TEXT("Could not spawn vehicle UI widget."));
...
void ATimeTrialPlayerController::OnPossess(APawn* InPawn)
{
	...
	VehiclePawn = CastChecked<AUnreal_projectPawn>(InPawn);
...
void ATimeTrialPlayerController::OnPawnDestroyed(AActor* DestroyedPawn)
{
	...
	if (ActorList.Num() > 0)
	{
		...
		if (AUnreal_projectPawn* RespawnedVehicle = GetWorld()->SpawnActor<AUnreal_projectPawn>(VehiclePawnClass, SpawnTransform))
		{
			// possess the vehicle
